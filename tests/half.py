...
def run_noshare(
    ...
    noshare_net = DistNet(model1, model2)
    ...
    for e in range(epochs):
        ...
        for i, (batch_1, batch_2) in enumerate(zip(dataloader_1, dataloader_2)):
            ...
            # yhats = noshare_net.forward(data_1, data_2)
            # loss = noshare_net.backward([label_1, label_2], yhats, nn.CrossEntropyLoss)
            # noshare_net.aggregate()
            # noshare_net.recompute_gradients()
            # loss_1, loss_2 = loss

            # Training
            yhat_1 = model1(data_1)
            loss_1 = nn.CrossEntropyLoss()(yhat_1, label_1)
            pred_1 = yhat_1.argmax(dim=1, keepdim=True)
            correct_1 += pred_1.eq(label_1.view_as(pred_1)).sum().item()
            totals_1 += len(data_1)
            yhat_2 = model2(data_2)
            loss_2 = nn.CrossEntropyLoss()(yhat_2, label_2)
        ...
        df.to_csv("results/halfdata.csv")
    return rows
