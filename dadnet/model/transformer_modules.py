import dadnet.model.transformer_util as util
from dadnet.model.transformer_util import mask_
...
class SelfAttentionGPT2(nn.Module):
    ...
    def __init__(self, emb, heads, mask=False):
        ...
        self.c_attn = nn.Linear(emb, 3 * emb, bias=False)

        # self.c_proj = Conv1D(emb, emb)
        self.c_proj = nn.Linear(emb, emb, bias=False)
...
class SelfAttentionWide(nn.Module):
    ...
    def __init__(self, emb, heads=8, mask=False):
        ...
        self.unifyheads = nn.Linear(heads * emb, emb, bias=False)
...
class TransformerBlock(nn.Module):
    def __init__(
        ...
        self.ff = nn.Sequential(
            nn.Linear(emb, ff_hidden_mult * emb, bias=False),
            nn.ReLU(),
            nn.Linear(ff_hidden_mult * emb, emb, bias=False),
        )
