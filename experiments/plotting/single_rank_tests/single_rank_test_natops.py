...
SEARCH = "*class*"
EXPNAME = "single_rank_test_natops"
RES_FOLDER = "results"
FIG_FOLDER = "figures"
COLORS = dict(
    light_blue="#a6cee3",
...
df = pd.DataFrame()
if os.path.exists("experiments/full_dfs/single_rank_test_natops_F.csv"):
    df = pd.read_csv("experiments/full_dfs/single_rank_test_natops.csv")
else:
    ...
    df.to_csv("experiments/full_dfs/single_rank_test_natops_new.csv", index=False)
...
plt.title("NATOPS- GRU - Effective Rank - Max Rank=32")
plt.legend(
    [
        "Output Layer",
        "2nd FC - 1024x1024",
        "1st FC - 6528 x 1024",
        "GRU-H2H Layer (dim 128)",
        "GRU-X2H Layer",
    ],
    loc="center",
...
plt.savefig("figures/single_rank_test_natops_effective_rank.png", bbox_inches="tight")
