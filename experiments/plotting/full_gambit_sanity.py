...
for DATASET in DATASETS:
    ...
    for i, mode in enumerate(MODE):
        ...
        sb.lineplot(x="epoch", y="test_acc_site_avg", data=sub_df)
    plt.ylabel("")
    plt.xlabel("")
    plt.title(DATASET)
    ...
    plt.legend(MODE)
    plt.savefig(
        "figures/full_sanity_%s_final_auc_epochs.png" % (DATASET), bbox_inches="tight"
    )
    plt.savefig(
        "figures/svg/full_sanity_%s_final_auc_epochs.svg" % (DATASET),
        bbox_inches="tight",
    )
